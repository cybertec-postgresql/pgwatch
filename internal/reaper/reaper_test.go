...
func TestReaper_LoadSources(t *testing.T) {
    ...
    source4 := sources.Source{Name: "Source 4", IsEnabled: true, Kind: sources.SourcePostgres, Group: "default"}
    ...
    // Strict: only explicitly listed groups are included
    r := NewReaper(ctx, &cmdopts.Options{
        SourcesReaderWriter: newReader,
        Sources: sources.CmdOpts{Groups: []string{"group1", "group2"}},
    })
    assert.NoError(t, r.LoadSources(ctx))
    assert.Equal(t, 2, len(r.monitoredSources), "Expected only explicitly grouped sources")

    r = NewReaper(ctx, &cmdopts.Options{
        SourcesReaderWriter: newReader,
        Sources: sources.CmdOpts{Groups: []string{"group1"}},
    })
    assert.NoError(t, r.LoadSources(ctx))
    assert.Equal(t, 1, len(r.monitoredSources), "Expected only group1 sources")

    // No groups specified → all enabled sources included
    r = NewReaper(ctx, &cmdopts.Options{SourcesReaderWriter: newReader})
    assert.NoError(t, r.LoadSources(ctx))
    assert.Equal(t, 4, len(r.monitoredSources), "Expected all sources when no groups are specified")
})
