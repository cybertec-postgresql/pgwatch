...
- **PW2_PGDATABASE** Config DB name. Default: pgwatch3
- **PW2_PGUSER** Config DB user. Default: pgwatch3
- **PW2_PGPASSWORD** Config DB password. Default: pgwatch3admin
...
- **PW2_DATASTORE** Backend for metric storage - [postgres|prometheus|graphite|json]. Default: postgres
...
- **PW2_ADHOC_NAME** Ad-hoc mode. Unique 'dbname' for data store. Default: adhoc
...
- **PW2_PROMETHEUS_NAMESPACE** Prefix for all non-process (thus Postgres) metrics. Default: "pgwatch3"
...
- **PW2_EMERGENCY_PAUSE_TRIGGERFILE** When the file exists no metrics will be temporarily fetched / scraped. Default: /tmp/pgwatch3-emergency-pause
...
- **PW2_WEBCERT** Enables use of own certificates for custom deployments. Default: '/pgwatch3/persistent-config/self-signed-ssl.pem'
- **PW2_WEBKEY** Enables use of own certificates for custom deployments. Default: '/pgwatch3/persistent-config/self-signed-ssl.key'
...
- **PW2_WEBUSER** Admin login. Default: pgwatch3
- **PW2_WEBPASSWORD** Admin password. Default: pgwatch3admin
...
- **PW2_AES_GCM_KEYPHRASE** Keyphrase for encryption/decpyption of connect string passwords.
- **PW2_AES_GCM_KEYPHRASE_FILE** File containing a keyphrase for encryption/decpyption of connect string passwords.
- **PW2_DATASTORE** Backend for metric storage - [postgres|graphite]. Default: postgres
- **PW2_PG_METRIC_STORE_CONN_STR** Postgres metric store connection string. Required when PW2_DATASTORE=postgres. Default: -
...
- **PW2_GRAFANAPASSWORD** Administrative user password. Default: pgwatch3admin
