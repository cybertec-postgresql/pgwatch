...
function start_pg {
  ...
  PW_USER=$(psql -U postgres -h localhost -p $port -XAtc "select count(*) from pg_roles where rolname = 'pgwatch3'")
  if [ $? -ne 0 ] || [ "$PW_USER" -ne 1 ]; then
    ...
    if [ $? -ne 0 ]; then
      echo "could not create pgwatch3 user on docker PG $full_ver on port $port"
      exit 1
    fi
  fi
