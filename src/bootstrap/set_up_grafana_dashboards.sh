...
psql -h /var/run/postgresql -f /pgwatch3/bootstrap/grafana_datasource.sql pgwatch3_grafana

for slug in $(ls --hide='*.md' /pgwatch3/grafana_dashboards/influxdb/v${GRAFANA_MAJOR_VER}) ; do

echo "inserting dashboard: $slug"
TITLE=$(cat /pgwatch3/grafana_dashboards/influxdb/v${GRAFANA_MAJOR_VER}/${slug}/title.txt)
JSON=$(cat /pgwatch3/grafana_dashboards/influxdb/v${GRAFANA_MAJOR_VER}/${slug}/dashboard.json)
