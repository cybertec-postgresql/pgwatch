...
on conflict (ma_metric_name)
do update set ma_metric_attrs = pgwatch3.metric_attribute.ma_metric_attrs || '{"metric_storage_name": "stat_statements", "prerequisite_extensions": ["pg_stat_statements"]}', ma_last_modified_on = now();

insert into pgwatch3.metric_attribute (ma_metric_name, ma_metric_attrs)
select 'stat_statements', '{"prerequisite_extensions": ["pg_stat_statements"]}'
on conflict (ma_metric_name)
do update set ma_metric_attrs = pgwatch3.metric_attribute.ma_metric_attrs || '{"prerequisite_extensions": ["pg_stat_statements"]}', ma_last_modified_on = now();
