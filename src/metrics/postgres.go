...
func (dmrw *dbMetricReaderWriter) UpdateMetric(metricName string, metric Metric) error {
	...
	WHERE name = $1`,
		metricName, db.NullIfZero(metric.SQLs), metric.InitSQL, metric.Description,
		metric.NodeStatus, db.NullIfZero(metric.Gauges), metric.IsInstanceLevel, metric.StorageName)
	return err
}
...
func (dmrw *dbMetricReaderWriter) UpdatePreset(presetName string, preset Preset) error {
	...
	_, err := dmrw.configDb.Exec(dmrw.ctx, sql, presetName, preset.Description, db.NullIfZero(preset.Metrics))
	return err
}
