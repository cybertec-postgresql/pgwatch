...
- [x] T002 Capture the baseline: `go test ./pkg/sinks/...` green, and record current `pgwatch --help` output for the **CON-006** behaviour-neutrality comparison in T038
...
- [x] T003 Create `pkg/sinks/feedback.go` with the `Feedbacker` interface and the `ErrFeedbackUnsupported` / `ErrNoFeedbackData` sentinels, copied verbatim from spec §4.1 including doc comments (**REQ-001**, **REQ-003**, **REQ-005**, **REQ-006**, **REQ-007**, **REQ-013**)
- [x] T004 Add `NoFeedback bool` to `sinks.CmdOpts` in `pkg/sinks/cmdopts.go` plus a `FeedbackEnabled()` accessor (**CFG-001**, **CFG-003**); see D-01 for why the flag is negated
- [x] T005 [P] Add the `fakeFeedbacker` test double to `pkg/sinks/feedback_test.go`: scriptable `CanFeedback` result, epoch, error, and per-method call counters (§6 Mocks). It drives every row of the §4.3 table without needing four real sinks
- [x] T006 [P] Add negative capability guards to `pkg/sinks/feedback_test.go` asserting `PrometheusWriter` and `JSONWriter` do **not** satisfy `Feedbacker`, and that both still satisfy `Writer` (**PRM-001**, **JSN-001**, **AC-010**)
- [x] T007 [P] Add the scope guard enforcing **AC-017**: a test (or CI step) that greps `Feedbacker|LastMeasurement|CanFeedback` across `--include=*.go` and fails if any non-test hit falls outside `pkg/sinks`
...
- [x] T008 [P] [US1] `pgxmock` unit tests for `CanFeedback` in `pkg/sinks/postgres_feedback_test.go`: non-empty pair → true; empty `sourceName` → false; empty `metricName` → false; feedback disabled → false; no lock taken (**PGS-002**, **PGS-008**)
...
- [x] T011 [US1] Integration test in `pkg/sinks/postgres_feedback_integration_test.go` using `testutil.SetupPostgresContainer()`, following the `pkg/reaper/database_integration_test.go` convention: real table via the sink's own `SyncMetric`/`AddOp` path, dropped in `t.Cleanup`. Cover the live epoch round-trip (**AC-002**), buffered-but-unflushed measurements excluded (**AC-013**, **REQ-011**), and a ≥ 30-partition table answering in under 100 ms (**PGS-004**, §6 Performance)
- [x] T012 [US1] Race test in `pkg/sinks/postgres_feedback_integration_test.go` (a real backend is needed; pgxmock is not concurrency-safe) (pattern: `pkg/sinks/prometheus_race_test.go`) running `CanFeedback` + `LastMeasurement` + `Write` + `SyncMetric` concurrently under `-race`; assert no race and that `LastMeasurement` does not block `SyncMetric` for the round-trip duration (**PGS-008**, **AC-014**, **E-10**)

### Implementation for US1

- [x] T013 [US1] Implement `PostgresWriter.CanFeedback` in `pkg/sinks/postgres.go`: gate on `pgw.opts.FeedbackEnabled()` and reject empty names; optimistic and lock-free, since `partitionMapMetric` is empty at process start (**PGS-002**, **PGS-008**, **CFG-002**, **REQ-004**)
- [x] T014 [US1] Implement `PostgresWriter.LastMeasurement` in `pkg/sinks/postgres.go` per the §9.1 sketch — build the §4.4 query with `pgx.Identifier{...}.Sanitize()` and bind parameters (**PGS-003**, **SEC-001**), bound by `pgw.opts.RetentionInterval` (**PGS-004**, **DAT-003**), convert via `UnixNano` (**PGS-005**), apply the 5 s default deadline (**CON-002**)
- [x] T015 [US1] Add the `isUndefinedTable` helper mapping `SQLSTATE 42P01` to `ErrFeedbackUnsupported` in `pkg/sinks/postgres.go` (**PGS-007**)
...
- [x] T020 [US2] Implement `MultiWriter.CanFeedback` in `pkg/sinks/multiwriter.go` (**REQ-020**, **REQ-021**)
...
- [x] T024 [US3] Regenerate stubs with `task proto` and confirm `UnimplementedReceiverServer` gained the `GetLastMeasurement` fallback (**PLT-002**). The `*.pb.go` files are committed under `api/pb/`, and CI fails if they go stale, so the regenerated stubs ship with this change
- [x] T025 [US3] Pass `*CmdOpts` into `NewRPCWriter` in `pkg/sinks/rpc.go` (mirroring `NewPostgresWriter`) so the sink can honour the kill switch; update the call site in `pkg/sinks/multiwriter.go` and every constructor call in `pkg/sinks/rpc_test.go` (§4.6)

### Tests for US3

- [x] T026 [P] [US3] Add the gRPC server doubles in `pkg/sinks/rpc_feedback_test.go`: a scriptable `feedbackReceiver` (settable epoch and status) and a `legacyReceiver` that inherits an Unimplemented `GetLastMeasurement`, each served on an ephemeral port behind `testutil.AuthInterceptor` (§6 gRPC server double). Kept local to the test rather than in `testutil.SetupRPCServers()`, whose servers are process-global and would share scripted state across tests
- [x] T027 [P] [US3] `bufconn` tests in `pkg/sinks/rpc_test.go` covering all five rows of §4.5: `OK` + positive epoch; `OK` + `EpochNs <= 0` → `ErrNoFeedbackData` (**RPC-007**, **E-09**); `codes.Unimplemented` → `ErrFeedbackUnsupported` and `CanFeedback` false thereafter with **zero** further round-trips (**AC-008**, **RPC-003**); `codes.NotFound` → `ErrNoFeedbackData` (**RPC-005**); `codes.Unavailable` → transport error with `CanFeedback` still true (**AC-009**, **E-08**)
...
- [x] T034 [P] Extend `pkg/sinks/doc.go` to mention the optional feedback capability and which sinks provide it (§11)
...
Task: "Add fakeFeedbacker double in pkg/sinks/feedback_test.go"
...
Task: "pgxmock CanFeedback tests in pkg/sinks/postgres_feedback_test.go"
...
- **This change ships no consumer.** T007's guard exists precisely so one cannot be added here by accident; if it fails, something outside `pkg/sinks` started calling the interface (**AC-017**, **CON-006**)
...
| # | Item | Status |
|---|---|---|
| 1 | Interface and sentinels match §4.1 | ✅ `pkg/sinks/feedback.go` |
...
| 8 | Feedback API unused outside `pkg/sinks` | ✅ `TestFeedbackStaysUnwired` |
